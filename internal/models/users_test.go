...
func testUserToOneOidcIssuerUsingOidcIssuer(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local User
	var foreign OidcIssuer

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, userDBTypes, false, userColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize User struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, oidcIssuerDBTypes, false, oidcIssuerColumnsWithDefault...); err != nil {
	...
	local.OidcIssuerID = foreign.ID
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.OidcIssuer().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadOidcIssuer(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.OidcIssuer == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.OidcIssuer = nil
	if err = local.L.LoadOidcIssuer(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.OidcIssuer == nil {
		t.Error("struct should have been eager loaded")
...
func testUserToOneSetOpOidcIssuerUsingOidcIssuer(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, oidcIssuerDBTypes, false, strmangle.SetComplement(oidcIssuerPrimaryKeyColumns, oidcIssuerColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, oidcIssuerDBTypes, false, strmangle.SetComplement(oidcIssuerPrimaryKeyColumns, oidcIssuerColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*OidcIssuer{&b, &c} {
		err = a.SetOidcIssuer(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.OidcIssuerID != x.ID {
			t.Error("foreign key was wrong value", a.OidcIssuerID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.OidcIssuerID))
		reflect.Indirect(reflect.ValueOf(&a.OidcIssuerID)).Set(zero)
		...
		if a.OidcIssuerID != x.ID {
			t.Error("foreign key was wrong value", a.OidcIssuerID, x.ID)
		}
	}
}
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Name`: `string`, `Email`: `string`, `OidcIssuerID`: `uuid`, `OidcSubject`: `string`, `CreatedAt`: `timestamptz`, `UpdatedAt`: `timestamptz`, `DeletedAt`: `timestamptz`}
	_           = bytes.MinRead
)
