...
func testTokensSoftDelete(t *testing.T) {
	t.Parallel()
...
func testTokensQuerySoftDeleteAll(t *testing.T) {
	t.Parallel()
	...
	if rowsAff, err := Tokens().DeleteAll(ctx, tx, false); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
func testTokensSliceSoftDeleteAll(t *testing.T) {
	t.Parallel()
...
func testTokensQueryDeleteAll(t *testing.T) {
	...
	if rowsAff, err := Tokens().DeleteAll(ctx, tx, true); err != nil {
		t.Error(err)
	} else if rowsAff != 1 {
		t.Error("should only have deleted one row, but affected:", rowsAff)
	}
...
var (
	tokenDBTypes = map[string]string{`ID`: `uuid`, `ResourceUrn`: `string`, `Token`: `string`, `CreatedAt`: `timestamptz`, `LastUsedAt`: `timestamptz`, `DeletedAt`: `timestamptz`}
	_            = bytes.MinRead
)
