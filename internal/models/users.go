...
type User struct {
	...
	OidcIssuerID string      `boil:"oidc_issuer_id" json:"oidc_issuer_id" toml:"oidc_issuer_id" yaml:"oidc_issuer_id"`
...
var UserTableColumns = struct {
	...
	Name:         "users.name",
	Email:        "users.email",
	OidcIssuerID: "users.oidc_issuer_id",
...
var UserWhere = struct {
	...
	OidcIssuerID whereHelperstring
	...
	OidcIssuerID: whereHelperstring{field: "\"users\".\"oidc_issuer_id\""},
	OidcSubject:  whereHelperstring{field: "\"users\".\"oidc_subject\""},
...
type userR struct {
	OidcIssuer *OidcIssuer `boil:"OidcIssuer" json:"OidcIssuer" toml:"OidcIssuer" yaml:"OidcIssuer"`
}
...
func (r *userR) GetOidcIssuer() *OidcIssuer {
	if r == nil {
		return nil
	}
	return r.OidcIssuer
...
var (
	userAllColumns            = []string{"id", "name", "email", "oidc_issuer_id", "oidc_subject", "created_at", "updated_at", "deleted_at"}
	userColumnsWithoutDefault = []string{"oidc_issuer_id", "oidc_subject", "created_at", "updated_at"}
	userColumnsWithDefault    = []string{"id", "name", "email", "deleted_at"}
...
// OidcIssuer pointed to by the foreign key.
func (o *User) OidcIssuer(mods ...qm.QueryMod) oidcIssuerQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"id\" = ?", o.OidcIssuerID),
	}
...
// LoadOidcIssuer allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (userL) LoadOidcIssuer(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	if singular {
		...
		args = append(args, object.OidcIssuerID)
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if a == obj.OidcIssuerID {
					continue Outer
				}
			}

			args = append(args, obj.OidcIssuerID)
	...
	query := NewQuery(
		...
		qm.WhereIn(`oidc_issuers.id in ?`, args...),
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load OidcIssuer")
	}

	var resultSlice []*OidcIssuer
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice OidcIssuer")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for oidc_issuers")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.OidcIssuerID == foreign.ID {
				local.R.OidcIssuer = foreign
				if foreign.R == nil {
					foreign.R = &oidcIssuerR{}
				}
...
// SetOidcIssuer of the user to the related item.
// Sets o.R.OidcIssuer to related.
// Adds o to related.R.Users.
func (o *User) SetOidcIssuer(ctx context.Context, exec boil.ContextExecutor, insert bool, related *OidcIssuer) error {
	var err error
	if insert {
	...
	o.OidcIssuerID = related.ID
	if o.R == nil {
		o.R = &userR{
			OidcIssuer: related,
		}
	} else {
		o.R.OidcIssuer = related
	}

	if related.R == nil {
		related.R = &oidcIssuerR{
			Users: UserSlice{o},
		}
	} else {
